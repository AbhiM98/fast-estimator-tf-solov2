...
def data_local(aws_path,local_path):
...
def aws_dataloader(aws_path,local_path="data",download_data=False,val_split=False):
    if aws_path =="" or aws_path is None:
            raise ValueError("Must specify at least one of s3 path")
    
    if not os.path.isabs(local_path):
       
        local_path = os.path.join(str(Path.home()),local_path)#os.path.join(os.path.abspath("."),local_path)
      
    val_split_no = 0.1
    # paths={}
    train_annotation = os.path.join(local_path, "labels", "train.json")
    val_annotation = os.path.join(local_path, "labels", "val.json")
    train_image_path = os.path.join(local_path,"train")
    val_image_path = os.path.join(local_path,"val")
    paths ={"train_annotation_path":train_annotation,"val_annotation_path":val_annotation,"train_image_path":train_image_path,"val_image_path":val_image_path}
    if download_data:
        # with ThreadPoolExecutor(max_workers=10) as executor:

        data_local(aws_path,local_path)
        val_split =0.1
        paths = validation_split(local_path,val_split)

    train_ds,val_ds=custom_data_pycoco_loader(paths["train_image_path"],paths['val_image_path'],paths["train_annotation_path"],paths['val_annotation_path'])
    # for x in train_ds:
    #     tf.print(next(x))
...
def validation_split(
    ...
    if not os.path.isdir(os.path.join(local_path, "train")):
        os.mkdir(os.path.join(local_path, "train"))
    if not os.path.isdir(os.path.join(local_path, "val")):
        os.mkdir(os.path.join(local_path, "val"))

    for path in paths:
        im_name = os.path.split(path)[-1]
        if im_name.split("_")[0] in training_name:
            os.rename(path, os.path.join(local_path, "train", im_name))
        elif im_name.split("_")[0] in validation_name:
            os.rename(path, os.path.join(local_path, "val", im_name))

    train_ann = {}
    val_ann = {}

    if len(glob.glob(os.path.join(local_path, "dataset.json"))) > 0:
        local_ann_file = os.path.join(local_path, "dataset.json")
    else:
        local_ann_file = os.path.join(local_path, "labels", "dataset.json")
    ...
    if not os.path.isdir(os.path.join(local_path, "labels")):
        os.mkdir(os.path.join(local_path, "labels"))
    ...
    with open(val_annotation, "w") as f:
        json.dump(val_ann, f)
    return paths
def custom_data_pycoco_loader(train_data,val_data,train_annotation_data,val_annotation_data):
    print(train_data)
    print(train_annotation_data)
    print(val_data)
    print(val_annotation_data)
    train_ds = mscoco.MSCOCODataset(Path(train_data),
                             annotation_file=Path(train_annotation_data),#os.path.join(annotation_data, "instances_train2017.json"),
                             caption_file=None,#os.path.join(annotation_data, "captions_train2017.json"),
                             keypoint_file=None,#os.path.join(annotation_data, "person_keypoints_train2017.json"),
                             include_bboxes=True,
                             include_masks=True,
                             include_captions=False,
                             include_keypoints=False,
                             replacement=True)
    val_ds = mscoco.MSCOCODataset(Path(val_data),
                            annotation_file=Path(val_annotation_data),#os.path.join(annotation_data, "instances_val2017.json"),
                            caption_file=None,#os.path.join(annotation_data, "captions_val2017.json"),
                            keypoint_file=None,#os.path.join(annotation_data, "person_keypoints_val2017.json"),
                            include_bboxes=True,#load_bboxes,
                            include_masks=True,#load_masks,
                            include_captions=False,
                            include_keypoints=False,
                            replacement=True)
    print(val_ds)
...
def solov2(input_shape=(None, None, 3), num_classes=80):
    ...
    resnet50 = tf.keras.applications.ResNet101(weights="imagenet", include_top=False, input_tensor=inputs, pooling=None)
...
class COCOMaskmAP(Trace):
    def __init__(self, data_dir, inputs=None, outputs="mAP", mode=None):
        ...
        # print(data_dir)
        with Suppressor():
            self.coco_gt = COCO(os.path.join(data_dir.replace('val', 'labels'), "val.json"))
        missing_category = [66, 68, 69, 71, 12, 45, 83, 26, 29, 30]
        category = [x for x in range(1, 91) if not x in missing_category]
