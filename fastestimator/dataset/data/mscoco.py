...
class MSCOCODataset(DirDataset):
    ...
    def __getitem__(self, index: Union[int, str]) -> Union[Dict[str, Any], np.ndarray, List[Any]]:
        ...
        while not has_data:
            ...
            if self.keypoints and not response["keypoint"]:
                has_keypoint = False
            # print("here",self.include_bboxes,has_box,has_mask)
    ...
    def _get_single_item(self, index: Union[int, str]) -> Union[Dict[str, Any], np.ndarray, List[Any]]:
        ...
        # print(os.path.splitext(os.path.basename(image)))
        # image_id = int(os.path.splitext(os.path.basename(image))[0])
        # self.get_ids =  self.coco.getImgIds()
        image_name =  os.path.basename(image)

        image_ids=self.instances.getImgIds()
        image_details = self.instances.loadImgs(image_ids)
        for img_d in image_details:
            if img_d["file_name"]==image_name:
                image_id = img_d["id"]
    ...
    def _populate_instance_data(self, data: Dict[str, Any], image_id: int) -> None:
        ...
        # print("x0",annotation_ids)
        # # if len(annotation_ids)==0:
        # #     annotation_ids=self.instances.getAnnIds()
        # print("x1",annotation_ids)
        if annotation_ids:
            # print("y")
